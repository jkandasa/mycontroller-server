...
if [[ "${TARGET_BINARY}" == "gateway" ]]; then # build gateway binary
  GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -v -tags=standalone -o mycontroller-gateway -ldflags "$LD_FLAGS" cmd/component/gateway/main.go

elif [[ "${TARGET_BINARY}" == "handler" ]]; then # build handler binary
  GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -v -tags=standalone -o mycontroller-handler -ldflags "$LD_FLAGS" cmd/component/handler/main.go
